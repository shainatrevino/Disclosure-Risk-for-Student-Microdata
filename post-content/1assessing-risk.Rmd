...
```
The resulting data frame shows the `r nrow(var_combos)` possible response combinations for our categorical indirect identifiers and the corresponding risk metrics. You can see that the first response combination is unique, whereas the second combination has 11 students with those responses. 
...
We already know from calculating the *k*-anonymity violations above that there are `r nrow(uniq)` unique individual responses in our data set. However, it is helpful to explore the pattern of unique responses to see which variables or response options are leading to high re-identification risk. 
...
The resulting data frame shows the risk metrics for the `r nrow(risky)` possible response combinations that are under our threshold. Similar to the above step, it can be helpful here to explore this data frame for patterns of risky responses to see if there are certain variables or response options that are most often leading to risky variable combinations. 
